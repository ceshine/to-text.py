...
parser = argparse.ArgumentParser(
    description='Convert HTML page to text using readability and html2text.')
parser.add_argument('-u', '--url', help='URL to convert', required=True)
parser.add_argument(
    '-s', '--sleep', help='Sleep X seconds between URLs (only in rss)', default=3)
parser.add_argument('-t', '--timeout',
                    help='Timeout for HTTP requests', default=30)
...
def custom_workaround_twitter(url):
    ...
    if hostname == "twitter.com" or hostname == "www.twitter.com":
        ...
        content = session_requests.get(url, headers=headers, proxies=PROXIES,
        ...
        content2 = session_requests.get(url, headers=headers, proxies=PROXIES,
...
def save_doc(text, title, url, rssDate=0):
    ...
    if rssDate:
        posttime = datetime.fromtimestamp(
            time.mktime(rssDate)).strftime("%Y%m%dT%H%M")
    else:
...
def save_gophermap(text, title, server, gophertype, filename, gopherport):
    ...
    if not os.path.exists(gmfile):
        with open(gmfile, "w") as gophermap:
            ...
            gophermap.write(
                "iThis article is available via Gopher. \t/\tlocalhost\t70\n")
